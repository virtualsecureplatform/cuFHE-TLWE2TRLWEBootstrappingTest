...
int main(){
    ...
    for (int test = 0; test < num_test; test++) {
        ...
        TLWElvl0 tlwe = tlweSymEncryptlvl0(p?DEF_μ:-DEF_μ,DEF_α,sk.key.lvl0);
        for(int i = 0;i<=DEF_n;i++) ct.lwe_sample_->data()[i] = tlwe[i];
        high_resolution_clock::time_point begin = high_resolution_clock::now();
        GateBootstrappingTLWE2TRLWElvl01NTT(cutrlwe,ct,st);
        high_resolution_clock::time_point end = high_resolution_clock::now();
        Synchronize();
        array<bool, DEF_N> p2 = trlweSymDecryptlvl1(cutrlwe.trlwehost, sk.key.lvl1);
        assert(static_cast<int>(p) == static_cast<int>(p2[0]));
